...
namespace Assets.Scripts.Quest.Heroes
{
    ...
    public class HeroCanvas : MonoBehaviour
    {
        ...
        void AddHero(Hero h, Game game)
        {
            ...
            transFrame.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top,
                (0.25f + offset) * UIScaler.GetPixelsPerUnit(), heroSize * UIScaler.GetPixelsPerUnit());
            transFrame.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, 0.25f * UIScaler.GetPixelsPerUnit(),
                heroSize * UIScaler.GetPixelsPerUnit());
            ...
            imageFrame.rectTransform.sizeDelta = new Vector2(heroSize * UIScaler.GetPixelsPerUnit(),
                heroSize * UIScaler.GetPixelsPerUnit());
            ...
            trans.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, (0.25f + offset) * UIScaler.GetPixelsPerUnit(),
                heroSize * UIScaler.GetPixelsPerUnit());
            ...
            trans.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, 0.25f * UIScaler.GetPixelsPerUnit(),
                heroSize * UIScaler.GetPixelsPerUnit());
            ...
            image.rectTransform.sizeDelta = new Vector2(heroSize * UIScaler.GetPixelsPerUnit() * 0.8f,
                heroSize * UIScaler.GetPixelsPerUnit() * 0.8f);
            if (game.gameType is MoMGameType)
            {
                image.rectTransform.sizeDelta = new Vector2(heroSize * UIScaler.GetPixelsPerUnit() * 0.9f,
                    heroSize * UIScaler.GetPixelsPerUnit() * 0.9f);
        ...
        public void UpdateImages()
        {
            ...
            foreach (Hero h in game.quest.heroes)
            {
                ...
                if (h.heroData != null)
                {
                    ...
                    Texture2D heroTex = ContentDataBase.FileToTexture(h.heroData.image);
                    Sprite heroSprite = null;
                    if (heroTex != null)
                    {
                        heroSprite = Sprite.Create(heroTex, new Rect(0, 0, heroTex.width, heroTex.height), Vector2.zero,
                            1);
                    }
                ...
                if (frameTex != null)
                {
                    frameSprite = Sprite.Create(frameTex, new Rect(0, 0, frameTex.width, frameTex.height), Vector2.zero,
                        1);
                }
        ...
        void HeroDiag(int id)
        {
            ...
            if (GameObject.FindGameObjectWithTag(Game.DIALOG) != null)
            {
                ...
                if (game.quest.eManager.CurrentValkyrieEvent != null &&
                    game.quest.eManager.CurrentValkyrieEvent.QEventQuestComponent.maxHeroes != 0)
                {
        ...
        public void EndSection()
        {
            ...
            // Check for validity
            if (heroCount < game.quest.qd.QuestIniComponent.minHero) return;
