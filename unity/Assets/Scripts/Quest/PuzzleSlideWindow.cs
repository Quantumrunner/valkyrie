...
namespace Assets.Scripts.Quest
{
    public class PuzzleSlideWindow
    {
        ...
        public PuzzleSlideWindow(ValkyrieEvent e)
        {
            ValkyrieEventData = e;
            Game game = Game.Get();
        ...
        public void CreateWindow()
        {
            ...
            transBg.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, UIScaler.GetPixelsPerUnit() * 2.5f,
                18f * UIScaler.GetPixelsPerUnit());
            transBg.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left,
                UIScaler.GetHCenter(-12f) * UIScaler.GetPixelsPerUnit(), 24f * UIScaler.GetPixelsPerUnit());
        ...
        public void DrawSlideFrame(Transform trans, float scale = 3f)
        {
            ...
            bLine[0].GetComponent<RectTransform>()
                .SetInsetAndSizeFromParentEdge(RectTransform.Edge.Bottom, -thick, thick);
            bLine[0].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, -thick,
                (scale * 6f * UIScaler.GetPixelsPerUnit()) + (2 * thick));

            bLine[1].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, -thick, thick);
            bLine[1].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, -thick,
                (scale * 6f * UIScaler.GetPixelsPerUnit()) + (2 * thick));

            bLine[2].GetComponent<RectTransform>()
                .SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, -thick, thick);
            bLine[2].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, -thick,
                (scale * 6f * UIScaler.GetPixelsPerUnit()) + (2 * thick));

            bLine[3].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Right,
                (scale * 2f * UIScaler.GetPixelsPerUnit()) - thick, thick);
            bLine[3].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, -thick,
                (scale * 2f * UIScaler.GetPixelsPerUnit()) + thick);

            bLine[4].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Right,
                (scale * 2f * UIScaler.GetPixelsPerUnit()) - thick, thick);
            bLine[4].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Bottom, -thick,
                (scale * 3f * UIScaler.GetPixelsPerUnit()) + thick);

            bLine[5].GetComponent<RectTransform>()
                .SetInsetAndSizeFromParentEdge(RectTransform.Edge.Right, -thick, thick);
            bLine[5].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top,
                (scale * 2f * UIScaler.GetPixelsPerUnit()) - thick,
                (scale * UIScaler.GetPixelsPerUnit()) + (2 * thick));

            bLine[6].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top,
                (scale * 2f * UIScaler.GetPixelsPerUnit()) - thick, thick);
            bLine[6].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Right, -thick,
                (scale * 2f * UIScaler.GetPixelsPerUnit()) + thick);

            bLine[7].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Bottom,
                (scale * 3f * UIScaler.GetPixelsPerUnit()) - thick, thick);
            bLine[7].GetComponent<RectTransform>().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Right, -thick,
                (scale * 2f * UIScaler.GetPixelsPerUnit()) + thick);
        }
        ...
        public void CreateBlock(PuzzleSlide.Block block, RectTransform pos, bool target = false)
        {
            ...
            transBg.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top,
                (block.ypos * 3f * UIScaler.GetPixelsPerUnit()) + 0.1f,
                ((block.ylen + 1) * 3f * UIScaler.GetPixelsPerUnit()) - 0.2f);
            transBg.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left,
                (block.xpos * 3f * UIScaler.GetPixelsPerUnit()) + 0.1f,
                ((block.xlen + 1) * 3f * UIScaler.GetPixelsPerUnit()) - 0.2f);
