    ...
    public class InvestigatorItems
    {
        ...
        public InvestigatorItems()
        {
            ...
            foreach (KeyValuePair<string, QuestComponent> kv in game.quest.qd.components)
            {
                ...
                if (itemQuestComponent != null && itemQuestComponent.starting &&
                    game.quest.itemSelect.ContainsKey(kv.Key)
            ...
            SortedList<string, SortedList<string, string>> itemSort =
            ...
            foreach (string category in itemSort.Keys)
            {
                ...
                {
                    Texture2D tex = ContentDataBase.FileToTexture(game.cd.items[item].image);
                    Sprite sprite = null;
                    if (tex != null)
                    {
                        sprite = Sprite.Create(tex, new Rect(0, 0, tex.width, tex.height), Vector2.zero, 1, 0,
