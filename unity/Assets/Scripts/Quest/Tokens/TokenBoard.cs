...
namespace Assets.Scripts.Quest.Tokens
{
    ...
    public class TokenBoard : MonoBehaviour
    {
        ...
        // Add a door
        public void Add(DoorBoardComponent d)
        {
            tc.Add(new TokenControl(d));
        }

        // Add a token
        public void Add(TokenBoardComponent t)
        {
            tc.Add(new TokenControl(t));
        }

        // Add a Ui
        public void Add(UIBoardComponent uiBoardComponent)
        {
            tc.Add(new TokenControl(uiBoardComponent));
        }

        // Monsters are only on the board during an event
        public void AddMonster(MonsterValkyrieEvent me)
        {
            Game game = Game.Get();
            int count = 0;
            ...
            {
                Texture2D newTex = ContentDataBase.FileToTexture(me.cMonster.image);
                AddPlacedMonsterImg("", newTex, 1, 1, me.QEventQuestComponent.location.x,
                    me.QEventQuestComponent.location.y);
            }
        ...
        public void AddPlacedMonsters(MonsterValkyrieEvent me, int count)
        {
            // Get monster placement image
            Texture2D newTex = ContentDataBase.FileToTexture(me.cMonster.imagePlace);
        ...
        public void AddPlacedMonsterImg(string place, Texture2D newTex, int sizeX, int sizeY, float posX = 0,
            float posY = 0)
        {
            Game game = Game.Get();
            Sprite iconSprite;
            ...
            if (game.gameType is D2EGameType)
            {
                ...
                iconFrame.sprite = Sprite.Create(frameTex, new Rect(0, 0, frameTex.width, frameTex.height),
                    Vector2.zero, 1);
            ...
            if (place.Length > 0)
            {
                ...
                iconCicle.sprite = Sprite.Create(circleTex, new Rect(0, 0, circleTex.width, circleTex.height),
                    Vector2.zero, 1);
        ...
        public void AddItem(Vector2 location, string item)
        {
            ...
            // Create the image
            Texture2D newTex = ContentDataBase.FileToTexture(game.cd.items[game.quest.itemSelect[item]].image);
