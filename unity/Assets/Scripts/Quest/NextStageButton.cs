...
namespace Assets.Scripts.Quest
{
    ...
    public class NextStageButton
    {
        ...
        public void Update()
        {
            ...
            StringKey phase;
            if (Game.Get().quest.phase == MoMPhase.horror)
            {
                phase = HORROR_STEP;
            }
            else if (Game.Get().quest.phase == MoMPhase.mythos)
            {
                phase = PHASE_MYTHOS;
            }
            else if (Game.Get().quest.phase == MoMPhase.monsters)
            {
                phase = MONSTER_STEP;
            }
            else
            ...
            ui.SetText(text);
            string_width = ui.GetStringWidth(text, UIScaler.GetMediumFont(), Game.Get().gameType.GetHeaderFont()) +
                           0.5f;
            ...
            string_width = ui.GetStringWidth(phase, UIScaler.GetMediumFont(), Game.Get().gameType.GetHeaderFont()) +
                           0.5f;
            ui.SetLocation(offset + ((UIScaler.GetRight(-4f) - offset - string_width) * 0.5f),
                UIScaler.GetBottom(-1.8f), string_width, 1.8f);
            ui.SetBGColor(bgColor);
            ui.SetFont(Game.Get().gameType.GetHeaderFont());
            ui.SetFontSize(UIScaler.GetMediumFont());
        ...
        public void Next()
        {
            ...
            {
                game.quest.log.Add(new LogEntry(new StringKey("val", "PHASE_MYTHOS").Translate()));
