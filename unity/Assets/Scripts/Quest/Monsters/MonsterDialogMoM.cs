...
namespace Assets.Scripts.Quest.Monsters
{
    ...
    public class MonsterDialogMoM : MonsterDialog
    {
        ...
        public MonsterDialogMoM(Monster m) : base(m)
        {
        }
        ...
        public override void CreateWindow()
        {
            ...
            if (game.quest.phase == MoMPhase.horror)
            {
        ...
        public static void DrawMonster(Monster monster, bool displayHealth = false)
        {
            Game game = Game.Get();

            Texture2D newTex = ContentDataBase.FileToTexture(monster.monsterData.image);
            ...
            {
                duplicateSprite =
                    Sprite.Create(dupeTex, new Rect(0, 0, dupeTex.width, dupeTex.height), Vector2.zero, 1);
            }
            ...
            trans.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, 1f * UIScaler.GetPixelsPerUnit(),
                8f * UIScaler.GetPixelsPerUnit());
            trans.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, 1f * UIScaler.GetPixelsPerUnit(),
                8f * UIScaler.GetPixelsPerUnit());
            ...
            icon.rectTransform.sizeDelta =
                new Vector2(8f * UIScaler.GetPixelsPerUnit(), 8f * UIScaler.GetPixelsPerUnit());
            ...
            if (duplicateSprite != null)
            {
                ...
                dupeFrame.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, 5f * UIScaler.GetPixelsPerUnit(),
                    UIScaler.GetPixelsPerUnit() * 4f);
                dupeFrame.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, 5f * UIScaler.GetPixelsPerUnit(),
                    4f * UIScaler.GetPixelsPerUnit());
                mImgDupe.AddComponent<CanvasRenderer>();

                iconDupe = mImgDupe.AddComponent<UnityEngine.UI.Image>();
                iconDupe.sprite = duplicateSprite;
                iconDupe.rectTransform.sizeDelta =
                    new Vector2(4f * UIScaler.GetPixelsPerUnit(), 4f * UIScaler.GetPixelsPerUnit());
            }
        ...
        private static void DrawMonsterHealth(Monster monster, UnityEngine.Events.UnityAction<Monster, bool> call)
        {
            UIElement ui = new UIElement();
        ...
        public static void Defeated(Monster monster)
        {
            Destroyer.Dialog();
            Game game = Game.Get();
        ...
        public static void MonsterDamageDec(Monster monster, UnityEngine.Events.UnityAction<Monster, bool> call)
        {
            monster.damage -= 1;
            if (monster.damage < 0)
            {
        ...
        public static void MonsterDamageInc(Monster monster, UnityEngine.Events.UnityAction<Monster, bool> call)
        {
            monster.damage += 1;
            if (monster.damage > monster.GetHealth())
            {
