    ...
    public class ZipManager : MonoBehaviour
    {
        ...
        static public void Extract(string target_path, string archive_name, Extract_mode mode)
        {
            ...
            try
            {
                ...
                if (mode == Extract_mode.ZIPMANAGER_EXTRACT_INI_TXT ||
                    mode == Extract_mode.ZIPMANAGER_EXTRACT_INI_TXT_PIC)
                {
                    zip.ExtractSelectedEntries("name = Quest.ini", null, target_path,
                        ExtractExistingFileAction.OverwriteSilently);
                    zip.ExtractSelectedEntries("name = Localization.*.txt", null, target_path,
                        ExtractExistingFileAction.OverwriteSilently);
                    ...
                    if (mode == Extract_mode.ZIPMANAGER_EXTRACT_INI_TXT_PIC)
                    {
                        ...
                        if (iniData.ContainsKey("image"))
                            zip.ExtractSelectedEntries("name = '" + iniData["image"] + "'", null, target_path,
                                ExtractExistingFileAction.OverwriteSilently);
                    }
                }
                ...
                {
                    zip.ExtractSelectedEntries("name = save.ini", null, target_path,
                        ExtractExistingFileAction.OverwriteSilently);
                    zip.ExtractSelectedEntries("name = image.png", null, target_path,
                        ExtractExistingFileAction.OverwriteSilently);
