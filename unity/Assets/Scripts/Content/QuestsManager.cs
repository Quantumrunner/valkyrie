...
namespace Assets.Scripts.Content
{
    public class QuestsManager
    {
        ...
        //   key : Quest name
        //   value : Quest object
        public Dictionary<string, Assets.Scripts.Content.QuestIniComponent> remote_quests_data = null;

        // Ini content for all local quests (only required when offline)
        //   key : Quest name
        //   value : Quest object
        public Dictionary<string, Assets.Scripts.Content.QuestIniComponent> local_quests_data = null;
        ...
        {
            remote_quests_data = new Dictionary<string, Assets.Scripts.Content.QuestIniComponent>();
            ...
            {
                HTTPManager.Get("https://drive.google.com/uc?id=13JEtzRQ1LcCAAhKluxii0tgKDW71XODV&export=download",
                    QuestsDownload_callback);
                quest_list_mode = QuestListMode.DOWNLOADING;
            }
            else if (game.gameType.TypeName() == "D2E")
            {
                HTTPManager.Get("https://drive.google.com/uc?id=1oa6NhKLUFn61RH1niPJzpFT4fG9iQFas&export=download",
                    QuestsDownload_callback);
                quest_list_mode = QuestListMode.DOWNLOADING;
            }
            else
        ...
        private void QuestsDownload_callback(string data, bool error, System.Uri uri)
        {
            ...
            {
                remote_quests_data.Add(quest_kv.Key, new Assets.Scripts.Content.QuestIniComponent(quest_kv.Value));
            }
        ...
        private void CheckLocalAvailability()
        {
            ...
            foreach (KeyValuePair<string, Assets.Scripts.Content.QuestIniComponent> quest_data in remote_quests_data)
            {
                if (localManifest.data.ContainsKey(quest_data.Key))
                {
                    quest_data.Value.downloaded = true;
                    quest_data.Value.update_available =
                        (localManifest.data[quest_data.Key]["version"] != quest_data.Value.version);
                }
            }
        }
        ...
        public void SortQuests()
        {
            ...
            quests_sorted_by_date =
                new SortedList<System.DateTime, string>(new DuplicateKeyComparer<System.DateTime>());
            ...
            {
                foreach (KeyValuePair<string, Assets.Scripts.Content.QuestIniComponent> quest_data in local_quests_data)
                {
            ...
            {
                if (game.stats != null && game.stats.scenarios_stats != null
                ) // we should wait for stats to be available
                {
                    foreach (KeyValuePair<string, Assets.Scripts.Content.QuestIniComponent> quest_data in
                        remote_quests_data)
                    {
                        string pkg_name = quest_data.Key.ToLower() + ".valkyrie";
                        if (game.stats.scenarios_stats.ContainsKey(pkg_name))
                        {
                            quests_sorted_by_rating.Add(game.stats.scenarios_stats[pkg_name].scenario_avg_rating,
                                quest_data.Key);
                            quests_sorted_by_avg_duration.Add(
                                game.stats.scenarios_stats[pkg_name].scenario_avg_duration, quest_data.Key);
                            quests_sorted_by_win_ratio.Add(game.stats.scenarios_stats[pkg_name].scenario_avg_win_ratio,
                                quest_data.Key);
                        }
                        else
                        {
                        ...
                        if (quest_data.Value.languages_name.Keys.Contains(game.currentLang))
                            quests_sorted_by_name.Add(quest_data.Value.languages_name[game.currentLang],
                                quest_data.Key);
                        else
                            quests_sorted_by_name.Add(quest_data.Value.languages_name[quest_data.Value.defaultLanguage],
                                quest_data.Key);
                        quests_sorted_by_difficulty.Add(quest_data.Value.difficulty, quest_data.Key);
                        quests_sorted_by_duration.Add(quest_data.Value.lengthMax, quest_data.Key);
        ...
        public Assets.Scripts.Content.QuestIniComponent GetQuestData(string key)
        {
            if (quest_list_mode == QuestListMode.ONLINE && !force_local_quest)
