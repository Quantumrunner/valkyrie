...
namespace Assets.Scripts.Save
{
    ...
    {
        public const string minValkyieVersion = "0.7.3";
        ...
        private static void SaveWithScreen(int num, bool quit = false)
        {
            ...
            try
            {
                ...
                float scale = 4f / 30f;
                Texture2D outTex = new Texture2D(Mathf.RoundToInt(screenSize.x * scale),
                    Mathf.RoundToInt(screenSize.y * scale), TextureFormat.RGB24, false);
        ...
        public static void Load(int num = 0)
        {
            ...
            try
            {
                if (File.Exists(SaveFile(num)))
                {
                    ...
                    ZipManager.Extract(valkyrieLoadPath, SaveFile(num),
                        ZipManager.Extract_mode.ZIPMANAGER_EXTRACT_FULL);
                    ...
                    QuestIniComponent q =
                        new QuestIniComponent(questLoadPath);
                    if (!q.valid)
                    {
                        ValkyrieDebug.Log(
                            "Error: save contains unsupported Quest version." + Environment.NewLine);
                        Destroyer.MainMenu();
                        return;
                    }
                    ...
                    {
                        ValkyrieDebug.Log(
                            "Error: save is from an old unsupported version." + Environment.NewLine);
                        Destroyer.MainMenu();
                        return;
                    }
