...
namespace Assets.Scripts.QuestEditor
{
    ...
    public class EditorComponent
    {
        ...
        private static readonly List<string> InternalPerilVariables = new List<string>
            {"$perilMinor11", "$perilm2value", "$perild1value", "$perilDeadly2Count"};
            ...
            foreach (VarTestsComponent tc in component.tests.VarTestsComponents)
            {
                if (tc is VarOperation)
                {
                    ...
                    if (component_index != (component.tests.VarTestsComponents.Count - 1)
                        ...
                        && component.tests.VarTestsComponents.FindIndex(component_index + 1,
                            x => x.GetClassVarTestsComponentType() ==
                                 VarTestsLogicalOperator.GetVarTestsComponentType()) != -1
                    )
                    {
                        ui = new UIElement(Game.EDITOR, scrollArea.GetScrollTransform());
                        ui.SetLocation(0.5f, offset, 1, 1);
                        ...
                        {
                            component.tests.moveComponent(tmp_index, false);
                            Update();
                        });
                    ...
                    if (component_index != 0
                        && component.tests.VarTestsComponents.FindLastIndex(component_index - 1,
                            x => x.GetClassVarTestsComponentType() ==
                                 VarTestsLogicalOperator.GetVarTestsComponentType()) != -1
                    )
                    {
                        ...
                        ui.SetButton(delegate
                        {
                            component.tests.moveComponent(tmp_index, true);
                            Update();
                        });
                        new UIElementBorder(ui, Color.yellow);
                    }
                ...
                if (tc is VarTestsParenthesis)
                {
                    ...
                    ui.SetButton(delegate
                    {
                        component.tests.Remove(tmp_index);
                        Update();
                    });
                    new UIElementBorder(ui, Color.red);
        ...
        public void ChangeSource()
        {
            UIWindowSelectionList select =
                new UIWindowSelectionList(SelectSource, new StringKey("val", "SELECT", CommonStringKeys.FILE));
        ...
        public void AddTestOp()
        {
            ...
            UIWindowSelectionListTraits select = new UIWindowSelectionListTraits(delegate(string s) { SelectAddOp(s); },
                new StringKey("val", "SELECT", VAR));
        ...
        public void AddAssignOp()
        {
            ...
            UIWindowSelectionListTraits select =
                new UIWindowSelectionListTraits(delegate(string s) { SelectAddOp(s, false); },
                    new StringKey("val", "SELECT", VAR));
        ...
        public static HashSet<string> ExtractVarsFromEvent(EventQuestComponent e)
        {
            ...
            {
                vars.Add(op.var);
                if (op.value.Length > 0 && op.value[0] != '#' && !char.IsNumber(op.value[0]) && op.value[0] != '-' &&
                    op.value[0] != '.')
                {
                    vars.Add(op.value);
                }
        ...
        public void SetTestOpp(VarOperation op)
        {
            ...
            UIWindowSelectionList select = new UIWindowSelectionList(delegate(string s) { SelectSetOp(op, s); },
                new StringKey("val", "SELECT", OP));
        ...
        public void SetValue(VarOperation op)
        {
            ...
            UIWindowSelectionListTraits select =
                new UIWindowSelectionListTraits(delegate(string s) { SelectSetValue(op, s); },
                    new StringKey("val", "SELECT", VALUE));
